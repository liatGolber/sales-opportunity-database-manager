    ...
    static class Program
    {
       ...
       Type.Missing, Type.Missing, Type.Missing);
        ...
        public static void RemovePackage(Package p)
        {
            ...
            packages.Remove(p);
            InsertUpdateOpp(p.ID, treatedAt: DateTime.Now);
        }

        public static bool PhoneUsed(string phone)
        {
            foreach (Opp o in opportunites)
                if (o.phone == phone)
                    return true;
        ...
        public static void InsertNewUser(string id, string name, string lName, string password, bool isAdmin)
        {
            ...
            object[,] values = (ess["Range"] as Excel.Range).Value2;
            userList.Add(new User() { ID = id, name = name, lastN = lName, password = password, isAdmin = isAdmin });
        ...
        public static void InsertUpdateOpp(string id, string name = "", string lName = "", string phone = "", string email = "", DateTime? treatedAt = null, string status = "", string treatedBy = "", string comment = "")
        {
            ...
            for (int i = 1; i <= values.GetLength(0); i++)
            {
                ...
                {
                    if (name != "")
                        values[i, 2] = op.name = name;
                    if (lName != "")
                        values[i, 3] = op.lastN = lName;
                    if (phone != "")
                        values[i, 4] = op.phone = phone;
                    if (email != "")
                        values[i, 5] = op.email = email;
                    if (status != "")
                        values[i, 6] = op.status = status;
                    if (treatedBy != "")
                    {
                        values[i, 7] = treatedBy;
                        op.treatedBy = GetUserByID(treatedBy);
                    }
                    if (treatedAt != null)
                        values[i, 8] = op.treatedAt = treatedAt.Value.Date;
                    if (comment != "")
                        values[i, 9] = op.comment = comment;
                    flag = false;
            ...
            if (flag)
            {
                ...
                if (id != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 1] = op.ID = id;
                if (name != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 2] = op.name = name;
                if (lName != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 3] = op.lastN = lName;
                if (phone != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 4] = op.phone = phone;
                if (email != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 5] = op.email = email;
                if (status != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 6] = op.status = status;
                if (treatedBy != "")
                {
                ...
                if (treatedAt != null)
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 8] = op.treatedAt = treatedAt.Value.Date;
                if (comment != "")
                    (ess["Sheet"] as Excel.Worksheet).Cells[i, 9] = op.comment = comment;
            }
            (ess["Book"] as Excel.Workbook).Save();
            CleanExcelEss(ref ess);
        ...
        public static void InsertUpdatePackage(string id, string phone, int type, bool close)
        {
            ...
            Dictionary<string, object> ess = MakeExcelEss(opportunitesDB, 2);
            object[,] values = (ess["Range"] as Excel.Range).Value2;
            packages.Add(new Package() { ID = id, lineNum = phone, packageType = type });
            //
            bool flag = true;
            for (int i = 1; i <= values.GetLength(0); i++)
            {
           ...
           (ess["Book"] as Excel.Workbook).Save();
            CleanExcelEss(ref ess);
            InsertUpdateOpp(id, treatedAt: DateTime.Now);
        }
        ...
        public static void MovetHistory(string id)
        {
            ...
            try
            {
                ...
                if (values[i, 1] != null) i++;
                foreach (Package p in packages.ToList())
                {
                    if (p.ID == id)
                    {
                        packages.Remove(p);
                        ...
                        (ess["Sheet"] as Excel.Worksheet).Cells[i, 4] = a.startD = DateTime.Now.Date;
                        (ess["Sheet"] as Excel.Worksheet).Cells[i, 5] = a.endD = DateTime.Now.Date.AddMonths(12);
                        (ess["Sheet"] as Excel.Worksheet).Cells[i++, 6] = a.hID = pin;
                        packages.Add(a);
                    }
                }
               (ess["Book"] as Excel.Workbook).Save();
        ...
        private static void GenerateRndOpp(int k)
        {
            ...
            //  string[] status = { "New(10%)", "First contact(20%)", "In process(30%)", "Proposal/Price Quote(50%)", "Negotiation/Review(90%)", "Closed won(100%)", "Closed lost(0%)" };
            string[] status = { "New(10%)", "First contact(20%)", "In process(30%)", "Proposal/Price Quote(50%)", "Negotiation/Review(90%)" };
            //string[] status = { "Closed won(100%)", "Closed lost(0%)" };
            #endregion
            //must have for excel handeling
            object[,] values = new object[k, 9];
            Random rnd = new Random();
            object[,] packVals = new object[k * rnd.Next(2, 4), 3];
            int pI = 0;
            //
            for (int i = 0; i < k; i++)
            {
                ...
                for (int j = 0; j < 9; j++)
                    s += rnd.Next(j == 0 ? 5 : 10);
                values[i, 0] = s;
                values[i, 1] = names[rnd.Next(0, names.Length)];
                values[i, 2] = lastName[rnd.Next(0, lastName.Length)];
                s = "05";
                for (int j = 0; j < 8; j++)
                    s += rnd.Next(10);
                values[i, 3] = s;
                values[i, 4] = values[i, 1].ToString() + "_" + values[i, 2].ToString() + "@" + Convert.ToChar(rnd.Next(Convert.ToInt32('A'), Convert.ToInt32('Z') + 1)) + "mail.com";
                values[i, 5] = status[rnd.Next(0, status.Length)];
                values[i, 6] = userList[rnd.Next(userList.Count)].ID;
                int sub = rnd.Next(31) * -1;
                DateTime d = DateTime.Now.AddDays(sub);
                values[i, 7] = d.Date;
                values[i, 8] = "don’t know what he wants in his life";
                int pN = rnd.Next(1, 4);
                for (int j = 0; j < pN && pI < packVals.GetLength(0); j++)
                {
                    packVals[pI, 0] = values[i, 0];
                    s = "05";
                    for (int t = 0; t < 8; t++)
                        s += rnd.Next(10);
                    packVals[pI, 1] = s;
                    packVals[pI++, 2] = rnd.Next(1, 4).ToString();
                }
            }
            Dictionary<string, object> ess = MakeExcelEss(opportunitesDB);
            Excel.Range c1 = (Excel.Range)(ess["Sheet"] as Excel.Worksheet).Cells[1, 1];
            Excel.Range c2 = (Excel.Range)(ess["Sheet"] as Excel.Worksheet).Cells[1 + values.GetLength(0) - 1, values.GetLength(1)];
            Excel.Range range = (Excel.Range)(ess["Sheet"] as Excel.Worksheet).get_Range(c1, c2);
            range.Value = values;
            (ess["Book"] as Excel.Workbook).Save();
            CleanExcelEss(ref ess);

            ess = MakeExcelEss(opportunitesDB, 2);
            c1 = (Excel.Range)(ess["Sheet"] as Excel.Worksheet).Cells[1, 1];
            c2 = (Excel.Range)(ess["Sheet"] as Excel.Worksheet).Cells[1 + packVals.GetLength(0) - 1, packVals.GetLength(1)];
            range = (Excel.Range)(ess["Sheet"] as Excel.Worksheet).get_Range(c1, c2);
            range.Value = packVals;
            (ess["Book"] as Excel.Workbook).Save();
            CleanExcelEss(ref ess);
            //
        }
        ...
        static void Main()
        {
            ...
            //GenerateRndOpp(700);
            //init();
            //foreach (Opp o in opportunites.ToList())
